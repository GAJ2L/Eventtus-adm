...
class PessoaList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $row2 = $this->form->addFields([new TLabel('Email:', null, '14px', null, '100%'),$email],[new TLabel('Email2:', null, '14px', null, '100%'),$email2]);
        ...
        $column_email2 = new TDataGridColumn('email2', 'Email2', 'left');
        $column_email3 = new TDataGridColumn('email3', 'Email3', 'left');
        $column_foto = new TDataGridColumn('foto', 'Foto', 'left');
        $column_token = new TDataGridColumn('token', 'Token', 'left');
        $column_idioma = new TDataGridColumn('idioma', 'Idioma', 'left');
        ...
        $this->datagrid->addColumn($column_email2);
        $this->datagrid->addColumn($column_email3);
        $this->datagrid->addColumn($column_foto);
        $this->datagrid->addColumn($column_token);
        $this->datagrid->addColumn($column_idioma);
        ...
        $container->add(TBreadCrumb::create(['Geral','Pessoas']));
    ...
    public function onSearch()
    {
        ...
        if (isset($data->email2) AND ( (is_scalar($data->email2) AND $data->email2 !== '') OR (is_array($data->email2) AND (!empty($data->email2)) )) )
        {

            $filters[] = new TFilter('email2', 'like', "%{$data->email2}%");// create the filter 
        }

        if (isset($data->email3) AND ( (is_scalar($data->email3) AND $data->email3 !== '') OR (is_array($data->email3) AND (!empty($data->email3)) )) )
        {

            $filters[] = new TFilter('email3', 'like', "%{$data->email3}%");// create the filter 
        }

        if (isset($data->foto) AND ( (is_scalar($data->foto) AND $data->foto !== '') OR (is_array($data->foto) AND (!empty($data->foto)) )) )
        {

            $filters[] = new TFilter('foto', 'like', "%{$data->foto}%");// create the filter 
        }

        if (isset($data->token) AND ( (is_scalar($data->token) AND $data->token !== '') OR (is_array($data->token) AND (!empty($data->token)) )) )
        {

            $filters[] = new TFilter('token', 'like', "%{$data->token}%");// create the filter 
        }

        if (isset($data->idioma) AND ( (is_scalar($data->idioma) AND $data->idioma !== '') OR (is_array($data->idioma) AND (!empty($data->idioma)) )) )
        {

            $filters[] = new TFilter('idioma', 'like', "%{$data->idioma}%");// create the filter 
        }
